...
<section class="show-products" style="display: flex; flex-wrap: wrap; justify-content: center;">
    ...
    <div class="box-container" style="width: 800px; display: flex; flex-wrap: wrap; justify-content: space-around;">
        <?php
        $select_products = mysqli_query($conn, "SELECT * FROM `exchange`") or die('query failed');
        if(mysqli_num_rows($select_products) > 0){
        ...
        <div class="box" style="width: calc(33.33% - 20px); margin: 10px; text-align: center; border: 2px solid #ddd; padding: 10px; border-radius: 10px; background-color: #f9f9f9;">
            ...
            <div class="name" style="margin-top: 10px; font-weight: bold;"><?php echo $fetch_products['exchanged_book_name']; ?></div>
        </div>
        <?php
            }
        }else{
